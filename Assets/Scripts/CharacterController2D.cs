...
public class CharacterController2D : MonoBehaviour
{
	...
	[SerializeField] float tempField;
	...
	public void Move2(float move, bool crouch, bool jump, bool hook)
	{
		...
		if (m_Grounded || m_AirControl)
		{
...

            {
				...
				Vector3 targetVelocity = new Vector2(m_Rigidbody2D.velocity.x + move, m_Rigidbody2D.velocity.y);
				//Vector3 targetVelocity = new Vector2(move * 10f, m_Rigidbody2D.velocity.y);

				m_Rigidbody2D.velocity = Vector3.SmoothDamp(m_Rigidbody2D.velocity, targetVelocity, ref m_Velocity, m_MovementSmoothing/tempField);
	...
	public void Move(float move, bool airborn)
	{
		if (Physics2D.OverlapCircle(m_WallCheckLeft.position, k_WalledRadius, m_WhatIsWall))
		{
		...
		//only control the player if grounded or airControl is turned on
{
			if (m_Rigidbody2D.velocity.x < tempField)
			{
				Debug.Log("stuff");
			}
			// Move the character by finding the target velocity
